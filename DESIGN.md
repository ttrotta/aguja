...
- Near-black page behind a raised panel, or muted linen behind a raised panel — never a flat
  single-plane background; the panel is always visibly a panel.
...
### Hierarchy
- **Display** (900, clamp(3.5rem, 10vw, 7.5rem), line-height 0.92): the "Aguja" wordmark on the
  landing hero. Appears once per page.
...
"scroll to explore" cue (a small pill with a drifting dot, the one deliberately familiar pattern on
this page) anchors its bottom edge. The next section begins exactly at the fold; nothing about it
...
the page itself. Inside it, three columns: left (document input + strategy controls, on
...
### Named Rules
**The Earned Glow Rule.** Violet glow marks the thread/needle motif and the current selection
only. A button does not glow at rest; glow is earned by being the active thing, not by being
interactive.
